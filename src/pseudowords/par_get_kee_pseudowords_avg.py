...
class Coercion:
    ...
    def _train(self, model, vec_targets, queries, targets1):
        ...
        with tqdm(total=5, desc="Train Loss", position=1) as loss_bar:
            for _ in trange(epoch, position=0, desc="Epoch", leave=True):
        ...
        # save checkpoints
        np.save(f"{CACHE}temp_z_arrays_mbart_{args.task_id}.npy", np.array(z_list))
        np.save(f"{CACHE}temp_loss_arrays_mbart_{args.task_id}.npy", np.array(loss_list))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task_id', type=int, required=True, help='Task ID for the current task')
    ...
    np.save(f"{DIR_OUT}pseudowords_comapp_{args.task_id}.npy", result)
