...
class Coercion:
    ...
    def coercion(self, group_no, group, k: int = 5):
        ...
        try:
            with open(DOC_PATH + "new_queries_bsbbert_" + str(group_no), "rb") as file:
    ...
    def _train(self, model, vec_targets, queries):
        ...
        target_idxs = [tokenizer.tokenize(input_id_list).index(NEW_TOKEN) for input_id_list in tokenizer.batch_decode(input_ids)] # [tokenizer.tokenize(query[0]).index(NEW_TOKEN) + 1 for query in queries]
        ...
        # save checkpoints
        np.save(CACHE + f"temp_z_arrays_bsbbert_{temp}.npy", np.array(z_list))
        np.save(CACHE + f"temp_loss_arrays_bsbbert_{temp}.npy", np.array(loss_list))
...
if __name__ == '__main__':
    ...
    if os.path.isfile(CACHE + f"temp_z_arrays_bsbbert_{temp}.npy"):
        z_list = np.load(CACHE + f"temp_z_arrays_bsbbert_{temp}.npy").tolist()
    if os.path.isfile(CACHE + f"temp_loss_arrays_bsbbert_{temp}.npy"):
        loss_list = np.load(CACHE + f"temp_loss_arrays_bsbbert_{temp}.npy").tolist()
    ...
    for group in tqdm(data[start:end], initial=start, total=len(data),
        ...
        np.save(DIR_OUT + f'pseudowords_comapp_bsbbert_{start}_{end}.npy', result)

        with open(DIR_OUT + f"order_bsbbert_{temp}.csv", "a+") as order_file:
